...
std::tuple<size_t, int, int, int> process_args(int argc, char* argv[]) {
    ...
    if (size_options.empty() || offset_options.empty() || iter_options.empty()) {
        ...
        return {100000000, 32, 10, 3}; // sensible defaults
    }
...
void warm_up_cache(const double* ptr, size_t size, int iterations) {
...
int main(int argc, char* argv[]) {
    ...
    for (int trial = 0; trial < trials; ++trial) {
        ...
        warm_up_cache(aligned_ptr, size, 5);   
        warm_up_cache(unaligned_ptr, size, 5);
